...
- **해결 방안 (`x-forwarded-host` 감지)**:
  ...
  - 이를 기반으로 스팀(`returnURL`) 및 라이엇(`callbackURL`)의 연동 주소를 실시간 동적으로 매핑하여 넘기도록 수정했습니다.
...
- `authRoutes.js`: `checkSteamAuth` 및 `checkRiotAuth` 미들웨어 내에서 `x-forwarded-host` 헤더를 기반으로 `returnURL`/`callbackURL` 동적 생성 로직 탑재.
